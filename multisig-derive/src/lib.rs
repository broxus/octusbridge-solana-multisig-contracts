...
#[darling(default, attributes(multisig_pack), forward_attrs(allow, doc, cfg))]
struct Opts {
    length: usize,
}

#[proc_macro_derive(MultisigPack, attributes(multisig_pack))]
pub fn derive_known_param_type(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    let input = syn::parse_macro_input!(input as syn::DeriveInput);
    impl_derive_multisig_pack(input).into()
}

fn impl_derive_multisig_pack(input: syn::DeriveInput) -> TokenStream {
